...
import com.great.happyness.ui.qrcode.IQrcodeListener;
...
import com.great.happyness.utils.UrlParse;
...
public class ConnectWifiActivity extends Activity implements IQrcodeListener, OnClickListener, Callback {
	...
	private String QRCODE_CAPTURE_BROADCAST = "act.qrcode.capture.result";
    ...
    private void initBroadcastReceiver() {
        ...
        intentFilter.addAction(QRCODE_CAPTURE_BROADCAST);

        registerReceiver(connRecv, intentFilter);
    }
    
    //@SuppressWarnings("static-access")
    private BroadcastReceiver connRecv = new BroadcastReceiver() {
		@Override
        public void onReceive(Context context, Intent intent) 
        {
            final String action = intent.getAction();
            if(action.equals(QRCODE_CAPTURE_BROADCAST)) {
            	inactivityTimer.onActivity();
            	
        		String qrcode = intent.getStringExtra("qrcode");
        		String stType = UrlParse.getValueByKey(qrcode, "hps");
        		int intType   = Integer.valueOf(stType).intValue(); 
        		String ssid   = UrlParse.getValueByKey(qrcode, "sid");
        		Log.w(TAG, "Got decode succeeded resultString:"+qrcode);
	        	...
	        	int ret = mWifiUtils.wifiConnect("\""+ssid+"\"", SysConfig.WIFI_AP_KEY, intType);
	        	Toast.makeText(ConnectWifiActivity.this, "ssid:"+ssid + 
	        			" type:"+intType + " ret:"+ret, Toast.LENGTH_SHORT).show();
	        	if(processBarPopWin==null) {
	            	int pw = (int) (WiseApplication.SCREEN_WIDTH*0.3);
	            	int ph = (int) (WiseApplication.SCREEN_WIDTH*0.8);
	        		processBarPopWin = new ProcessBarPopWin(ConnectWifiActivity.this, pw, ph);
	        		processBarPopWin.showAtLocation(findViewById(R.id.radar_ll), Gravity.CENTER, 0, 0);
	        	}
            }
            else if (action.equals(WifiManager.SCAN_RESULTS_AVAILABLE_ACTION)) //wifi已成功扫描到可用wifi
            {
	...
	private void initCamera(SurfaceHolder surfaceHolder) {
		...
		if (mHandler == null) {
			mHandler = new CaptureActivityHandler(this, 
					getViewfinderView(), decodeFormats, characterSet);
		}
	}
	...
	public void onClick(View v) {
		...
		switch(v.getId())
		{
			case R.id.bt_qr_capture:
				//startActivity(new Intent().setClass(ConnectWifiActivity.this, WifiActivity.class));
	...
	@Override
	public void callQrcode(final long decodeTime, Result obj, Bitmap barcode) {
		// TODO Auto-generated method stub
		String resultString = obj.getText();

		final Intent intent=new Intent(QRCODE_CAPTURE_BROADCAST);
		intent.putExtra("qrcode", resultString);
		sendBroadcast(intent);
	}

	@Override
	public void msgQrcode(int msg) {
		// TODO Auto-generated method stub
